...
export default function FuturePlanningScreen() {
  const [currentMonth, setCurrentMonth] = useState(DateTime.now().toFormat('yyyy-MM'));
  ...
  const goToNextMonth = () => {
    const nextMonth = DateTime.fromFormat(currentMonth, 'yyyy-MM').plus({ months: 1 });
    setCurrentMonth(nextMonth.toFormat('yyyy-MM'));
  };

  const goToPreviousMonth = () => {
    const previousMonth = DateTime.fromFormat(currentMonth, 'yyyy-MM').minus({ months: 1 });
    setCurrentMonth(previousMonth.toFormat('yyyy-MM'));
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.navigationContainer}>
        <Button title="Previous Month" onPress={goToPreviousMonth} />
        <Text style={styles.currentMonth}>
          {DateTime.fromFormat(currentMonth, 'yyyy-MM').toFormat('MMMM yyyy')}
        </Text>
        <Button title="Next Month" onPress={goToNextMonth} />
      </View>

      {/* Calendar for date selection */}
      <Calendar
        markingType="simple"
        current={`${currentMonth}-01`} // Dynamically set the current month
        markedDates={{
